...
const CouponEditPage: React.FC = () => {
  ...
  const isNewCoupon = id === 'new';

  // State
  const [saving, setSaving] = useState(false);
  // We'll keep coupon in state for future use if needed
  const [_coupon, _setCoupon] = useState<Coupon | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchedCoupon = MOCK_COUPONS.find(c => c.id === id);
    
    if (fetchedCoupon) {
      _setCoupon(fetchedCoupon);
      
      // Update form values
      const formValues: Partial<CouponFormValues> = {
        name: fetchedCoupon.name,
        description: fetchedCoupon.description,
        code: fetchedCoupon.code,
        type: fetchedCoupon.type,
        value: fetchedCoupon.value,
        applicabilityScope: fetchedCoupon.applicabilityScope,
        startDate: new Date(fetchedCoupon.startDate),
        status: fetchedCoupon.status,
        priority: fetchedCoupon.priority,
        isAutomaticallyApplied: fetchedCoupon.isAutomaticallyApplied,
        isOneTimeUse: fetchedCoupon.isOneTimeUse,
        customerUsageLimit: fetchedCoupon.customerUsageLimit,
        minimumOrderAmount: fetchedCoupon.minimumOrderAmount,
      };
      
      if (fetchedCoupon.endDate) {
        formValues.endDate = new Date(fetchedCoupon.endDate);
      }
  ...
  }, [id, isNewCoupon, form, navigate, toast, _setCoupon]);
  ...
  const onSubmit = (values: CouponFormValues) => {
    ...
    setTimeout(() => {
      ...
      toast({
        title: `Coupon ${isNewCoupon ? 'created' : 'updated'}`,
        description: `The coupon has been successfully ${isNewCoupon ? 'created' : 'updated'}.`,
      });
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            {isNewCoupon ? 'Create a new coupon code for your store' : 'Edit coupon details and settings'}
          </p>
        </div>
                    ...
                    <AlertDescription>
                      {!isNewCoupon && _coupon ? (
                        <div className="text-sm mt-2">
                          <p>Created: {new Date(_coupon.createdAt).toLocaleDateString()}</p>
                          <p>Times used: {_coupon.usageCount}</p>
                        </div>
                      ) : (
